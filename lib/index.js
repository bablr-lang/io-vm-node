...
function* __print(instrs) {
  const co = new Coroutine(getStreamIterator(instrs));
...
function* __evaluate(strategy, options) {
  ...
  const co = new Coroutine(getStreamIterator(print_(strategy())));
...
export const evaluate = (strategy, options = {}) =>
  new StreamIterable(__evaluate(strategy, options));
